...
DataGrid::DataGrid() : _SortMode(ref new Sort(SortMode::None, 0)), _Rows(ref new Vector<IVectorView<Platform::String^>^>()), _SelectedRow(nullptr), _SortedRows(ref new Vector<IVectorView<Platform::String^>^>()), _OriginalRows(nullptr), _Columns(ref new Vector<ColumnDefinition^>()), _ColumnsTitles(ref new Vector<Platform::String^>), _RowsBackgrounds(ref new Vector<Rectangle^>()), _RowsVectorChangedToken(EventRegistrationToken())
{
    InitializeComponent();
}
