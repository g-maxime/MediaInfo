//
// DataGrid.xaml.cpp
// Implementation of the DataGrid class
//
...
DataGrid::DataGrid() : _Rows(ref new Vector<IVectorView<Platform::String^>^>()), _SortedRows(ref new Vector<IVectorView<Platform::String^>^>()), _OriginalRows(nullptr), _Columns(ref new Vector<ColumnDefinition^>()), _ColumnsTitles(ref new Vector<Platform::String^>), _RowsBackgrounds(ref new Vector<Rectangle^>()), _SelectedRow(0), _RowsVectorChangedToken(EventRegistrationToken())
{
    InitializeComponent();
}
...
void DataGrid::Cell_Clicked(Object^ Sender, TappedRoutedEventArgs^ Event)
{
    ...
    if (_SelectedRow<_RowsBackgrounds->Size)
    {
        if (_SelectedRow%2==0)
            _RowsBackgrounds->GetAt(_SelectedRow)->Fill=ref new SolidColorBrush(Windows::UI::Colors::WhiteSmoke);
        else
            _RowsBackgrounds->GetAt(_SelectedRow)->Fill=ref new SolidColorBrush(Windows::UI::Colors::White);
    }
