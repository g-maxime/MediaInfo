//
// SheetEditDialog.xaml.cpp
// Implémentation de la classe SheetEditDialog
//
...
SheetEditDialog::SheetEditDialog(Platform::String^ Title, Sheet^ Content) : _Sheet(Content)
{
    ...
    {
        Add_Field(It->Title, It->Length, It->StreamKind, It->Parameter);
    }
    ...
    AddButton->Click+=ref new RoutedEventHandler([this](Object^, RoutedEventArgs^) {
        Add_Field(L"New Field", 0, 0, L"FileName");
    });
}

void SheetEditDialog::Add_Field(Platform::String^ Name, size_t Width, size_t StreamKind, Platform::String^ Parameter)
{
    SolidColorBrush^ TransparentBrush=ref new SolidColorBrush(Windows::UI::Colors::White);
    TransparentBrush->Opacity=0;
    ...
    SheetEditFieldControl^ FieldControl=ref new SheetEditFieldControl(Name, Width, StreamKind, Parameter);
...
void SheetEditDialog::ContentDialog_PrimaryButtonClick(ContentDialog^ Sender, ContentDialogButtonClickEventArgs^ Event)
{
    ...
    for (Object^ Obj : FieldList->Items)
    {
        ...
        Fields->Append(ref new Field(FieldControl->Get_Name(), FieldControl->Get_StreamKind(), FieldControl->Get_Parameter(), FieldControl->Get_Width()));
    }
