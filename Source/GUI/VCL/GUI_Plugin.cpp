...
bool __fastcall TPluginF::Configure()
{
    ...
    SourceURL = __T("https://mediaarea.net/download/snapshots/binary/mediainfo-graph-plugin/20210706/MediaInfo_GraphPlugin_1.0_Windows.exe"); // TODO: Get from downloaded prefs
    if (SourceURL.empty())
    {
        MessageBox(NULL, __T("Unable to find installer for the requested plugin, please download it manually from the MediaInfo download page."), __T("Error"), MB_OK);
        return false;
